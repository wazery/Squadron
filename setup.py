...
METADATA = {
    ...
    'version':          '0.0.1',
    'license':          'GPL',
    'description':      'Blast those pesky aliens by taking control of two space cannons and blasting them to teeny bits.',
    'author':           'Islam Wazeyn',
    'author_email':     'al.wazery@gmail.com',
    'url':              'http://pyweek.org/e/Squadron',
    'classifiers':      [
...
_DATA_DIR = os.path.join('Squadron', 'data')
data = []
...
gfx_dirs = [os.path.join(f2.replace(_DATA_DIR, 'gfx')) for f2 in data]
PACKAGEDATA['package_data'] = {'Squadron': gfx_dirs}

sound_dirs = [os.path.join(f2.replace(_DATA_DIR, 'sound')) for f2 in data]
PACKAGEDATA['package_data'] = {'Squadron': sound_dirs}
...
src = []
add_files(src,os.walk('Squadron'))
